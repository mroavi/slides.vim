...
"syn region markdownCodeBlock start="^\n\( \{4,}\|\t\)" end="^\ze \{,3}\S.*$" keepend " mrv: disable tab-based code blocks
...
" mrv: copied `markdownAutomaticLink` syn statement above and appended `concealends`
exe 'syn region markdownAutomaticLink matchgroup=markdownUrlDelimiter start="<\%(\w\+:\|[[:alnum:]_+-]\+@\)\@=" end=">" keepend oneline' . s:concealends
" mrv: my custom regions
exe 'syn region markdownTitle matchgroup=markdownTitleDelimiter start="==\S\@=" end="\S\@<===\|^$" contains=markdownLineStart,@Spell' . s:concealends
...
hi def link markdownTitleDelimiter        markdownTitle
